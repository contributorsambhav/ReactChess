    ...
    <div className="flex h-screen items-center justify-center w-screen overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col h-full w-full">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold pt-6 sm:pt-8 md:pt-10 lg:pt-12 pb-4 sm:pb-6 md:pb-8 text-center text-white z-10 animate-fade-in px-4">
            Select a Puzzle
          </h1>
            ...
            <div className="max-w-7xl mx-auto space-y-8 sm:space-y-10 md:space-y-12">
              {sortedCategories.map((category, categoryIndex) => (
                <div 
                  key={category} 
                  className={`${
                    animate ? `animate-slide-in ${getDelayClass(categoryIndex)}` : ""
                  }`}
                >
                  {/* Category Header */}
                  <div className="mb-4 sm:mb-5 md:mb-6">
                    <h3 className="text-white text-center text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold animate-fade-in drop-shadow-lg" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.8)' }}>
                      {category.toUpperCase()}
                    </h3>
                    {getCategoryDescription(category) && (
                      <p className="text-center text-sm sm:text-base md:text-lg text-gray-200 font-normal mt-1 sm:mt-2 drop-shadow-md" style={{ textShadow: '1px 1px 3px rgba(0,0,0,0.8)' }}>
                        {getCategoryDescription(category)}
                      </p>
                    )}
                  </div>
                  ...
                  <div className={`grid ${getGridClass(groupedPuzzles[category].length)} gap-3 sm:gap-4 md:gap-5 lg:gap-6`}>
                    {groupedPuzzles[category].map((puzzle, index) => (
                      <div
                        ...
                        className="w-full"
                      >
                        <div className="group h-full min-h-[120px] sm:min-h-[130px] md:min-h-[140px] flex flex-col justify-center transition-all duration-300 border-2 border-white/20 hover:border-cyan-400/80 hover:bg-gradient-to-br hover:from-cyan-500/20 hover:to-blue-600/20 transform hover:scale-[1.03] hover:shadow-2xl hover:shadow-cyan-500/30 bg-gray-900/70 backdrop-filter backdrop-blur-xl p-4 sm:p-5 md:p-6 rounded-xl shadow-2xl cursor-pointer">
                          <Link
                            to={`/puzzle/${puzzle.id}`}
                            className="text-white group-hover:text-cyan-300 text-base sm:text-lg md:text-xl lg:text-2xl text-center block font-bold drop-shadow-md transition-all duration-300 group-hover:drop-shadow-[0_0_8px_rgba(34,211,238,0.6)]"
